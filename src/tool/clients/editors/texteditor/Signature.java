...
//  static final Color     WHITE         = Display.getDefault().getSystemColor(SWT.COLOR_WHITE);
  ...
  public int maxHeight(GraphicsContext gc) {
//    Font f = gc.getFont();
//    gc.setFont(FONT);
...
//    gc.setFont(f);
  ...
  public void paint(int width, GraphicsContext gc) {
//    int maxWidth = maxWidth(entries, gc);
//    selectedEntry = null;
//    paint(width - (maxWidth + BORDER), 0, entries, gc);
  }

  public int paint(int x, int y, Vector<SignatureEntry> entries, GraphicsContext gc) {
//    startX = x;
//    Font f = gc.getFont();
...
//  private int maxWidth(Vector<SignatureEntry> entries, GraphicsContext gc) {
//    int width = 0;
//    Font f = gc.getFont();
//    gc.setFont(FONT);
//    for (SignatureEntry entry : entries) {
