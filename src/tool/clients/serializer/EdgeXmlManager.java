...
public class EdgeXmlManager {
    ...
    public Element createEdgeXmlElement(Vector<Object> intermediatePoints, String ref, Vector<Object>ports){
        ...
        Element edgeElement = xmlHandler.createXmlElement(SerializerConstant.TAG_NAME_EDGE);
        ...
        Element intermediatePointsNode = xmlHandler.createXmlElement(SerializerConstant.TAG_NAME_INTERMEDIATE_POINTS);
        for(Object intermediatePointObject : intermediatePoints){
            Vector<Object> points = (Vector<Object>) intermediatePointObject;
            Element intermediatePoint = xmlHandler.createXmlElement(SerializerConstant.TAG_NAME_INTERMEDIATE_POINT);
    ...
    private Element getEdgesElement(Element diagramNode){
        return xmlHandler.getChildWithTag(diagramNode, SerializerConstant.TAG_NAME_EDGES);
    }
    ...
    private void handleEdge(FmmlxDiagram fmmlxDiagram, Element diagramElement, Edge<?> edge) {
            Node edges = xmlHandler.getChildWithTag(diagramElement, SerializerConstant.TAG_NAME_EDGES);
                ...
                for (int j = 0 ; j< edgeList.getLength(); j++) {
                    if(edgeList.item(j).getNodeType()==Node.ELEMENT_NODE) {
                        ...
                        if (edge instanceof FmmlxAssociation) {
                            ...
                            if(edgeElement.getAttribute(SerializerConstant.ATTRIBUTE_NAME).equals(name) &&
                                    edgeElement.getAttribute(SerializerConstant.ATTRIBUTE_TYPE).equals(SerializerConstant.EdgeType.ASSOCIATION)){
                                setDirectionsAndIntermediatePoints(fmmlxDiagram, edge, edgeElement);
                                break;
                            }
                        } else if (edge instanceof DelegationEdge){
                            if(edgeElement.getAttribute(SerializerConstant.ATTRIBUTE_REFERENCE).equals(edge.getPath()) &&
                                    edgeElement.getAttribute(SerializerConstant.ATTRIBUTE_TYPE).equals(SerializerConstant.EdgeType.DELEGATION)){
                                setDirectionsAndIntermediatePoints(fmmlxDiagram, edge, edgeElement);
                                break;
                            }
                        } else if (edge instanceof FmmlxLink) {
                            if(edgeElement.getAttribute(SerializerConstant.ATTRIBUTE_REFERENCE).equals(edge.getPath()) &&
                                    edgeElement.getAttribute(SerializerConstant.ATTRIBUTE_TYPE).equals(SerializerConstant.EdgeType.LINK)){
                                setDirectionsAndIntermediatePoints(fmmlxDiagram, edge, edgeElement);
                                break;
                            }
                        } else if (edge instanceof InheritanceEdge) {
                            if(edgeElement.getAttribute(SerializerConstant.ATTRIBUTE_REFERENCE).equals(edge.getPath()) &&
                                    edgeElement.getAttribute(SerializerConstant.ATTRIBUTE_TYPE).equals(SerializerConstant.EdgeType.INHERITANCE)){
                                setDirectionsAndIntermediatePoints(fmmlxDiagram, edge, edgeElement);
                                break;
                            }
                        } else if (edge instanceof RoleFillerEdge) {
                            if(edgeElement.getAttribute(SerializerConstant.ATTRIBUTE_REFERENCE).equals(edge.getPath()) &&
                                    edgeElement.getAttribute(SerializerConstant.ATTRIBUTE_TYPE).equals(SerializerConstant.EdgeType.ROLEFILLEREDGE)){
                                setDirectionsAndIntermediatePoints(fmmlxDiagram, edge, edgeElement);
                            }
                        }
                    }
    ...
    private void setDirectionsAndIntermediatePoints(FmmlxDiagram fmmlxDiagram, Edge edge, Element edgeElement) {
        edge.getSourceNode().setDirectionForEdge(edge.sourceEnd, true,
                PortRegion.valueOf(edgeElement.getAttribute(SerializerConstant.ATTRIBUTE_SOURCE_PORT)));
        edge.getTargetNode().setDirectionForEdge(edge.targetEnd, false,
                PortRegion.valueOf(edgeElement.getAttribute(SerializerConstant.ATTRIBUTE_TARGET_PORT)));
        Node intermediatePointsNode = xmlHandler.getChildWithTag(edgeElement, SerializerConstant.TAG_NAME_INTERMEDIATE_POINTS);
