...
//  static Font                        diagramFont       ;//= new Font(XModeler.getXModeler().getDisplay(), new FontData("Courier New", 12, SWT.NO));
	static Font diagramFontFX;// = new Font(XModeler.getXModeler().getDisplay(), new FontData("Courier New",
								// 12, SWT.NO));
//  static Font                        diagramItalicFont ;//= new Font(XModeler.getXModeler().getDisplay(), new FontData("Courier New", 12, SWT.ITALIC)); 
	static Font diagramItalicFontFX;// = new Font(XModeler.getXModeler().getDisplay(), new FontData("Courier New",
									// 12, SWT.NO));
	// static java.awt.Font diagramFont_AWT = new java.awt.Font("Courier New",
...
//  }
	 ...
	 * Unfortunately, hi-dpi changes the font size, so we need to get that factor
	 * out of the dimensions again. The value sent to XMF are those AS IF the font
	 * size were at 100%
	 */
...
//      public void run() {
		...
		// true);
		// double awtWidth = diagramFont_AWT.getStringBounds(text.strValue(),
		// frc).getWidth()+2;
		// double awtHeight = diagramFont_AWT.getStringBounds(text.strValue(),
		// frc).getHeight();

		// Value italics = message.args[1];
		javafx.geometry.Point2D extent = textDimension(text.strValue(), diagramFontFX);

		// FontDescriptor myDescriptor =
		// FontDescriptor.createFrom(DiagramClient.diagramFont).setHeight(12 * 100 /
		// XModeler.getDeviceZoomPercent());
		...
		// (extent.x*100/XModeler.getDeviceZoomPercent())
...
//  }
	...
	private void inflateBox(String parentId, Node node) {
		...
		newBox(parentId, id, x, y, width, height, curve, top, right, bottom, left, lineRed, lineGreen, lineBlue,
	...
	private void inflateTool(String diagramId, String groupId, Node tool) {
		...
		if (toolType.equals("EdgeCreationTool"))
			...
			System.err.println("unknown type of tool: " + toolType);
	}
	...
	private void inflateEdgeElement(String id, Node node) {
		if (node.getNodeName().equals("Waypoint"))
			...
			System.err.println("Unknown type of edge element " + node.getNodeName());
	}
	...
	private void inflateEllipse(String parentId, Node node) {
		...
		int fillGreen = Integer.parseInt(XModeler.attributeValue(node, "fillGreen"));
		int fillBlue = Integer.parseInt(XModeler.attributeValue(node, "fillBlue"));
		newEllipse(parentId, id, x, y, width, height, showOutline, lineRed, lineGreen, lineBlue, fillRed, fillGreen,
				fillBlue);
	}
	...
	private void inflateLabel(String edgeId, Node node) {
		...
		newLabel(edgeId, id, text, pos, x, y, editable, underline, condense, red, green, blue, border, borderRed,
	...
	private void inflateMultilineText(String parentId, Node node) {
		...
		int fillBlue = Integer.parseInt(XModeler.attributeValue(node, "fillBlue"));
		String font = XModeler.attributeValue(node, "font");
		newMultilineText(parentId, id, text, x, y, width, height, editable, lineRed, lineGreen, lineBlue, fillRed,
				fillGreen, fillBlue, font);
	}
	...
	public void inflateXML(Document doc) {
		...
		if (diagramClients.getLength() == 1) {
			...
			System.err.println("expecting exactly 1 diagram client got: " + diagramClients.getLength());
	}
...
//  public void minimize(CTabFolderEvent event) {}
	...
	private void move(Message message) {
		...
		// System.err.println("DiagramClient->move(" + id.strValue() +", "+ x.intValue
		// +", "+ y.intValue +")");
		for (Diagram diagram : diagrams)
	...
	private void newBox(String parentId, String id, int x, int y, int width, int height, int curve, boolean top,
			boolean right, boolean bottom, boolean left, int lineRed, int lineGreen, int lineBlue, int fillRed,
			int fillGreen, int fillBlue) {
		for (Diagram diagram : diagrams) {
			diagram.newBox(parentId, id, x, y, width, height, curve, top, right, bottom, left, lineRed, lineGreen,
	...
	private void newEdge(String parentId, String id, String sourceId, String targetId, int refx, int refy,
			int sourceHead, int targetHead, int lineStyle, int red, int green, int blue, int sourceX, int sourceY,
	...
	private void newShape(Message message) {
		...
		newShape(parentId, id, x, y, width, height, showOutline, lineRed, lineGreen, lineBlue, fillRed, fillGreen,
				fillBlue, points);
	}

	private void newEllipse(final String parentId, final String id, final int x, final int y, final int width,
			final int height, final boolean showOutline, final int lineRed, final int lineGreen, final int lineBlue,
			final int fillRed, final int fillGreen, final int fillBlue) {
//    runOnDisplay(new Runnable() {
//      public void run() {
		for (Diagram d : diagrams)
			d.newEllipse(parentId, id, x, y, width, height, showOutline, lineRed, lineGreen, lineBlue, fillRed,
					fillGreen, fillBlue);
//      }
//    });
	}

	private void newShape(final String parentId, final String id, final int x, final int y, final int width,
			final int height, final boolean showOutline, final int lineRed, final int lineGreen, final int lineBlue,
			final int fillRed, final int fillGreen, final int fillBlue, final int[] points) {
//    runOnDisplay(new Runnable() {
//      public void run() {
		for (Diagram d : diagrams)
			d.newShape(parentId, id, x, y, width, height, showOutline, lineRed, lineGreen, lineBlue, fillRed, fillGreen,
					fillBlue, points);
//      }
//    });
	}
...
//      });
			...
			System.err.println("cannot find diagram " + diagramId);
	}
	...
	private void newImage(final String parentId, final String id, final String fileName, final int x, final int y,
...
//    });
	...
	private void newLabel(String parentId, String id, String text, String position, int x, int y, Boolean editable,
			Boolean underline, Boolean condense, int red, int green, int blue, boolean border, int borderRed,
			int borderGreen, int borderBlue, String font, int arrow, boolean hidden, boolean fill) {
		for (Diagram diagram : diagrams) {
			for (Edge edge : diagram.getEdges()) {
				if (edge.getId().equals(parentId)) {
					edge.addLabel(id, text, position, x, y, editable, underline, condense, red, green, blue, border,
							borderRed, borderGreen, borderBlue, font, arrow, hidden, fill);
				}
			}
		}
	}
	...
	private void newMultilineText(final String parentId, final String id, final String text, final int x, final int y,
			final int width, final int height, final boolean editable, final int lineRed, final int lineGreen,
			final int lineBlue, final int fillRed, final int fillGreen, final int fillBlue, final String font) {
//    runOnDisplay(new Runnable() {
//      public void run() {
		for (Diagram d : diagrams)
			d.newMultilineText(parentId, id, text, x, y, width, height, editable, lineRed, lineGreen, lineBlue, fillRed,
					fillGreen, fillBlue, font);
//      }
//    });
	}
	...
	private void newNode(Message message) {
		...
		newNode(parentId.strValue(), id.strValue(), x.intValue, y.intValue, width.intValue, height.intValue,
	...
	private void newText(final String parentId, final String id, final String text, final int x, final int y,
			final boolean editable, final boolean underline, final boolean italicise, final int red, final int green,
			final int blue) {
		for (Diagram diagram : diagrams) {
	...
	private void newTool(Message message) {  //TODO: all of this is redundant and terrible
		...
		newTool(diagramId.strValue(), groupId.strValue(), label.strValue(), toolId.strValue(), isEdge.boolValue,
				icon.strValue());
	}
	...
	private void newToggle(Message message) {
		...
		newToggle(diagramId.strValue(), groupId.strValue(), label.strValue(), toolId.strValue(), state.boolValue,
				iconTrue.strValue(), iconFalse.strValue());
	}
...
//      });
	...
	private void newTool(final String diagramId, final String groupId, final String label, final String toolId,
			final boolean isEdge, final String icon) {
		if (getDiagram(diagramId) != null) {
//      runOnDisplay(new Runnable() {
//        public void run() {
			Diagram diagram = getDiagram(diagramId);
...
//      });
	...
	private void newToggle(final String diagramId, final String groupId, final String label, final String toolId,
			final boolean state, final String iconTrue, final String iconFalse) {
		if (getDiagram(diagramId) != null) {
//      runOnDisplay(new Runnable() {
//        public void run() {
			Diagram diagram = getDiagram(diagramId);
...
//      });
	...
	private void newAction(final String diagramId, final String groupId, final String label, final String toolId,
			final String icon) {
		if (getDiagram(diagramId) != null) {
//      runOnDisplay(new Runnable() {
//        public void run() {
...
//      });
	 ...
	 * // final int time = 2500; // runOnDisplay(new Runnable() { // public void
	 * run() { // Runnable timer = new Runnable() { // public void run() { //
...
//    });
	...
	private void showEdges(final String id, final boolean top, final boolean bottom, final boolean left,
			final boolean right) { // Bj�rn
//    runOnDisplay(new Runnable() {
//      public void run() {
		for (Diagram diagram : diagrams)
...
//      public void run() {
		...
		for (String tabId : tabs.keySet()) {
			if (tabId.equals(id.strValue()))
				DiagramClient.tabPane.getSelectionModel().select(tabs.get(tabId));
		}
//      }
//    });
	}
