	...
	protected void align() {
		if (intermediatePoints.size() < 2) {
			if (startNode.getDirectionForEdge(this, true).isHorizontal())
				intermediatePoints
						.add(new Point2D((startNode.getCenterX() + endNode.getCenterX()) / 2, startNode.getCenterY()));
			else
				intermediatePoints
	...
	public void setPointAtToBeMoved(Point2D mousePoint) {
		...
		if (hitLine != null) {
			if (hitLine > 0 && hitLine < points.size() - 2) {
				...
				movementDirectionHorizontal = points.get(pointToBeMoved).getX() == points.get(pointToBeMoved + 1)
						.getX();
			} else if (hitLine == 0) {
		...
		/*
		 * // If there is a Point at p, then that is to be moved // otherwise one is
		 * created on the spot. // that point's index is stored temporarily while the
		 * mouse is dragged pointToBeMoved = -1; for (int i = 1; i < points.size() - 1;
		 * i++) { // can't drag first nor last Point2D edgePoint = points.get(i); if
		 * (distance(mousePoint, edgePoint) < DEFAULT_TOLERANCE) { pointToBeMoved = i; }
		 * } if (pointToBeMoved == -1) { // not pressed on an existing node for (int i =
		 * 0; i < points.size() - 1; i++) { // System.err.println("distance=" +
		 * distance(mousePoint, points.get(i), points.get(i+1))); if
		 * (distance_OLD(mousePoint, points.get(i), points.get(i + 1)) <
		 * DEFAULT_TOLERANCE) { pointToBeMoved = i + 1; // i is the point before the
		 * node. We will insert a new one, which will be number i+1 } } if
		 * (pointToBeMoved != -1) { // make sure we found one Point2D newPoint = new
		 * Point2D(mousePoint.getX(), mousePoint.getY());
