...
public abstract class Edge implements CanvasElement {
	...
	public Edge(int id, FmmlxObject startNode, FmmlxObject endNode, Vector<Point2D> intermediatePoints,
		...
		if (intermediatePoints == null || intermediatePoints.size() < 1) {
			// this.intermediatePoints.add(new Point2D(startNode.getX()+
			// startNode.getWidth() / 2, startNode.getY()+startNode.getHeight()/2));
			double middleX = (startNode.getWidth() + startNode.getX() / 2 + endNode.getX() + endNode.getWidth()) / 2;
			this.intermediatePoints.add(new Point2D(middleX, startNode.getPointForEdge(this, true).getY() + startNode.getHeight() / 2));
			this.intermediatePoints.add(new Point2D(middleX, endNode.getPointForEdge(this, false).getY() + endNode.getHeight() / 2));
			// this.intermediatePoints.add(new Point2D(endNode.getX()+endNode.getWidth()/2,
	...
	public void paintOn(GraphicsContext g, int xOffset, int yOffset, FmmlxDiagram fmmlxDiagram) {
		if (!layoutingFinishedSuccesfully) {
			...
			// for (EdgeLabel label : labels) label.paintOn(g, xOffset, yOffset,
			// fmmlxDiagram);
			g.setFill(Color.RED);
			g.fillText(
			...
			if (pointToBeMoved != -1) {
				final double R = 1.5;
				g.fillOval(points.get(pointToBeMoved).getX() - R, points.get(pointToBeMoved).getY() - R, 2 * R, 2 * R);
			}
	...
	public boolean isHit(Point2D p, Double tolerance) {
		...
		for (int i = 0; i < points.size() - 1; i++) {
			// System.err.println("distance=" + distance(p, points.get(i),
			// points.get(i+1)));
	...
	private double distance(Point2D testPoint, Point2D lineStart, Point2D lineEnd) { // some fancy math copied from the
																						// old diagram
	...
	public void movePoints(double newStartX, double newStartY, double newEndX, double newEndY) {
		...
		// double newX = (oldPoint.getX() - oldStartX) / (oldEndX - oldStartX) *
		// (newEndX - newStartX) + newStartX;
		// double newY = (oldPoint.getY() - oldStartY) / (oldEndY - oldStartY) *
		// (newEndY - newStartY) + newStartY;
	...
	// movePoints(newStartX, newStartY, points.lastElement().getX(),
	// points.lastElement().getY());
	// }
	//
	// public void moveEndPoint(double newEndX, double newEndY) {
	// movePoints(points.firstElement().getX(), points.firstElement().getY(),
	// newEndX, newEndY);
	// }
...
.clear();
	...
	public void setPointAtToBeMoved(Point2D mousePoint) {
		...
		if (pointToBeMoved == -1) { // not pressed on an existing node
			for (int i = 0; i < points.size() - 1; i++) {
				// System.err.println("distance=" + distance(mousePoint, points.get(i),
				// points.get(i+1)));
				if (distance(mousePoint, points.get(i), points.get(i + 1)) < DEFAULT_TOLERANCE) {
					pointToBeMoved = i + 1; // i is the point before the node. We will insert a new one, which will be
											// number i+1
				}
			}
