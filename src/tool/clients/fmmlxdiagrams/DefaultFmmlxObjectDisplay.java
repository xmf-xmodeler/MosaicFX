...
//		object.requiresReLayout = false;
		...
		for (FmmlxAttribute att : object.getOwnAttributes()) {
			...
			NodeLabel attLevelLabel = new NodeLabel(Pos.BASELINE_CENTER, 7, attY, Color.WHITE, att.level == -1? Color.WHITE : Color.BLACK, att, changeAttLevelAction, att.level == -1 ? " " : att.level + ""); //PM "c" used for now for non-intrinsic attributes
