...
public class DefaultFmmlxObjectDisplay extends AbstractFmmlxObjectDisplay {
	...
	public void layout(Map<DiagramDisplayProperties, Boolean> diagramToolBarProperties) {
		...
		if(issues.size() > 0 && diagramToolBarProperties.get(DiagramDisplayProperties.SHOWCONSTRAINTREPORTS)) {
		...
		for (FmmlxAttribute att : object.getOtherAttributes()) {
			if(diagramToolBarProperties.get(DiagramDisplayProperties.SHOWDERIVEDATTRIBUTES)) {
			attY += lineHeight;
		...
		if (diagramToolBarProperties.get(DiagramDisplayProperties.SHOWOPERATIONS) && opsSize > 0) {
			yAfterOpsBox = currentY + opsBoxHeight;
			group.addNodeElement(opsBox);
			for (FmmlxOperation o : object.getOwnOperations()) {
				if(diagramToolBarProperties.get(DiagramDisplayProperties.SHOWGETTERSANDSETTERS) || !(o.getName().startsWith("set") || o.getName().startsWith("get"))) {
					opsY += lineHeight;
			...
			for (FmmlxOperation o : object.getOtherOperations()) {
				if(diagramToolBarProperties.get(DiagramDisplayProperties.SHOWGETTERSANDSETTERS) || !(o.getName().startsWith("set") || o.getName().startsWith("get"))) {
					if(diagramToolBarProperties.get(DiagramDisplayProperties.SHOWDERIVEDOPERATIONS)) {
					opsY += lineHeight;
...
//					if(diagram.getObjectByPath(ofPath) != null && 
		...
		if (diagramToolBarProperties.get(DiagramDisplayProperties.SHOWCONSTRAINTS) && constraintSize > 0) {
		...
		if (diagramToolBarProperties.get(DiagramDisplayProperties.SHOWSLOTS) && slotSize > 0) {
...
//		double lineHeight = textHeight + EXTRA_Y_PER_LINE;
		...
		if (diagramToolBarProperties.get(DiagramDisplayProperties.SHOWOPERATIONVALUES) && opvSize > 0) {
...
//		object.handlePressedOnNodeElement(object.lastClick, diagram);
	...
	private int countOperationsToBeShown(Map<DiagramDisplayProperties, Boolean> diagramToolBarProperties) {
		int counter=0;
		for (FmmlxOperation o : object.getOwnOperations()) {
			if(diagramToolBarProperties.get(DiagramDisplayProperties.SHOWGETTERSANDSETTERS) ||  !(o.getName().startsWith("set") || o.getName().startsWith("get"))) {
			counter++;	
	...
	private int countAttributesToBeShown(Map<DiagramDisplayProperties, Boolean> diagramToolBarProperties) {
		return (diagramToolBarProperties.get(DiagramDisplayProperties.SHOWDERIVEDATTRIBUTES)?1:0)*object.getOtherAttributes().size() + object.getOwnAttributes().size();
	}
	...
	private double calculateNeededWidth(FmmlxDiagram diagram, Map<DiagramDisplayProperties, Boolean> diagramToolBarProperties) {
...
//		//determine maximal width of operations
		if (diagramToolBarProperties.get(DiagramDisplayProperties.SHOWOPERATIONS)) {
			for (FmmlxOperation o : object.getOwnOperations()) {
				if(diagramToolBarProperties.get(DiagramDisplayProperties.SHOWGETTERSANDSETTERS)  ||  !(o.getName().startsWith("set") || o.getName().startsWith("get"))) {
		...
		if (diagramToolBarProperties.get(DiagramDisplayProperties.SHOWSLOTS) && object.slots.size() > 0) {
		...
		if (diagramToolBarProperties.get(DiagramDisplayProperties.SHOWOPERATIONVALUES)) {
		...
		if (diagramToolBarProperties.get(DiagramDisplayProperties.SHOWCONSTRAINTS)) {
			for (Constraint con : object.getConstraints()) {
