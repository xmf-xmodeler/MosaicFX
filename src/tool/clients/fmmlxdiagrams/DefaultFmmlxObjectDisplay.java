...
public class DefaultFmmlxObjectDisplay extends AbstractFmmlxObjectDisplay {
	...
	public void layout(Map<DiagramToolBarProperties, Boolean> diagramToolBarProperties) {
		...
		if(issues.size() > 0 && diagramToolBarProperties.get(DiagramToolBarProperties.CONSTRAINTREPORTS)) {
		...
		for (FmmlxAttribute att : object.getOtherAttributes()) {
			if(diagramToolBarProperties.get(DiagramToolBarProperties.DERIVEDATTRIBUTES)) {
			attY += lineHeight;
		...
		if (diagramToolBarProperties.get(DiagramToolBarProperties.OPERATIONS) && opsSize > 0) {
			yAfterOpsBox = currentY + opsBoxHeight;
			group.addNodeElement(opsBox);
			for (FmmlxOperation o : object.getOwnOperations()) {
				if(diagramToolBarProperties.get(DiagramToolBarProperties.GETTERSANDSETTERS) || !(o.getName().startsWith("set") || o.getName().startsWith("get"))) {
					opsY += lineHeight;
			...
			for (FmmlxOperation o : object.getOtherOperations()) {
				if(diagramToolBarProperties.get(DiagramToolBarProperties.GETTERSANDSETTERS) || !(o.getName().startsWith("set") || o.getName().startsWith("get"))) {
					if(diagramToolBarProperties.get(DiagramToolBarProperties.DERIVEDOPERATIONS)) {
					opsY += lineHeight;
...
//					if(diagram.getObjectByPath(ofPath) != null && 
		...
		if (diagramToolBarProperties.get(DiagramToolBarProperties.CONSTRAINTS) && constraintSize > 0) {
		...
		if (diagramToolBarProperties.get(DiagramToolBarProperties.SLOTS) && slotSize > 0) {
...
//		double lineHeight = textHeight + EXTRA_Y_PER_LINE;
		...
		if (diagramToolBarProperties.get(DiagramToolBarProperties.OPERATIONVALUES) && opvSize > 0) {
...
//		object.handlePressedOnNodeElement(object.lastClick, diagram);
	...
	private int countAttributesToBeShown(Map<DiagramToolBarProperties, Boolean> diagramToolBarProperties) {
		return (diagramToolBarProperties.get(DiagramToolBarProperties.DERIVEDATTRIBUTES)?1:0)*object.getOtherAttributes().size() + object.getOwnAttributes().size();
	}
...
//		//determine maximal width of operations
		if (diagramToolBarProperties.get(DiagramToolBarProperties.OPERATIONS)) {
			for (FmmlxOperation o : object.getOwnOperations()) {
				if(diagramToolBarProperties.get(DiagramToolBarProperties.GETTERSANDSETTERS)  ||  !(o.getName().startsWith("set") || o.getName().startsWith("get"))) {
		...
		if (diagramToolBarProperties.get(DiagramToolBarProperties.SLOTS) && object.slots.size() > 0) {
		...
		if (diagramToolBarProperties.get(DiagramToolBarProperties.OPERATIONVALUES)) {
		...
		if (diagramToolBarProperties.get(DiagramToolBarProperties.CONSTRAINTS)) {
			for (Constraint con : object.getConstraints()) {
