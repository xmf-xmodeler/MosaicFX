...
public class DefaultFmmlxObjectDisplay extends AbstractFmmlxObjectDisplay {
	...
	public void layout(Map<DiagramsDisplayProperty, Boolean> diagramDisplayProperties) {
		...
		if(issues.size() > 0 && diagramDisplayProperties.get(DiagramsDisplayProperty.CONSTRAINTREPORTS)) {
		...
		for (FmmlxAttribute att : object.getOtherAttributes()) {
			if(diagramDisplayProperties.get(DiagramsDisplayProperty.DERIVEDATTRIBUTES)) {
			attY += lineHeight;
		...
		if (!object.hidden && diagramDisplayProperties.get(DiagramsDisplayProperty.OPERATIONS) && opsSize > 0) {
			yAfterOpsBox = currentY + opsBoxHeight;
			group.addNodeElement(opsBox);
			for (FmmlxOperation o : object.getOwnOperations()) {
				if(diagramDisplayProperties.get(DiagramsDisplayProperty.GETTERSANDSETTERS) || !(o.getName().startsWith("set") || o.getName().startsWith("get"))) {
					opsY += lineHeight;
			...
			for (FmmlxOperation o : object.getOtherOperations()) {
				if(diagramDisplayProperties.get(DiagramsDisplayProperty.GETTERSANDSETTERS) || !(o.getName().startsWith("set") || o.getName().startsWith("get"))) {
					if(diagramDisplayProperties.get(DiagramsDisplayProperty.DERIVEDOPERATIONS)) {
						opsY += lineHeight;
...
//					if(diagram.getObjectByPath(ofPath) != null && 
		...
		if (diagramDisplayProperties.get(DiagramsDisplayProperty.CONSTRAINTS) && constraintSize > 0) {
		...
		if (diagramDisplayProperties.get(DiagramsDisplayProperty.SLOTS) && slotSize > 0) {
...
//		double lineHeight = textHeight + EXTRA_Y_PER_LINE;
		...
		if (diagramDisplayProperties.get(DiagramsDisplayProperty.OPERATIONVALUES) && opvSize > 0) {
...
//		object.handlePressedOnNodeElement(object.lastClick, diagram);
	...
	private int countOperationsToBeShown(Map<DiagramsDisplayProperty, Boolean> diagramDisplayProperties) {
		int counter=0;
		for (FmmlxOperation o : object.getOwnOperations()) {
			if(diagramDisplayProperties.get(DiagramsDisplayProperty.GETTERSANDSETTERS) ||  !(o.getName().startsWith("set") || o.getName().startsWith("get"))) {
			counter++;	
	...
	private int countAttributesToBeShown(Map<DiagramsDisplayProperty, Boolean> diagramDisplayProperties) {
		return (diagramDisplayProperties.get(DiagramsDisplayProperty.DERIVEDATTRIBUTES)?1:0)*object.getOtherAttributes().size() + object.getOwnAttributes().size();
	}
	...
	private double calculateNeededWidth(FmmlxDiagram diagram, Map<DiagramsDisplayProperty, Boolean> diagramDisplayProperties) {
...
//		//determine maximal width of operations
		if (diagramDisplayProperties.get(DiagramsDisplayProperty.OPERATIONS)) {
			for (FmmlxOperation o : object.getOwnOperations()) {
				if(diagramDisplayProperties.get(DiagramsDisplayProperty.GETTERSANDSETTERS)  ||  !(o.getName().startsWith("set") || o.getName().startsWith("get"))) {
		...
		if (diagramDisplayProperties.get(DiagramsDisplayProperty.SLOTS) && object.slots.size() > 0) {
		...
		if (diagramDisplayProperties.get(DiagramsDisplayProperty.OPERATIONVALUES)) {
		...
		if (diagramDisplayProperties.get(DiagramsDisplayProperty.CONSTRAINTS)) {
			for (Constraint con : object.getConstraints()) {
