...
public class UploadConfig extends CustomDialog<UploadConfig.Result>{
	...
	private Label uriLabel = new Label("uri");
	private Label userLabel = new Label("user");
	private Label passwordLabel = new Label("password");

	
	private TextField uriTextfield = new TextField();
	...
	private void layout() {
		grid.add(uriLabel,0,1);
		grid.add(userLabel, 0, 2);
		grid.add(passwordLabel, 0, 3);
		
		grid.add(uriTextfield,1,1);
	...
	private void setResult() {
		setResultConverter(dlgBtn -> {
			if (dlgBtn != null && dlgBtn.getButtonData() == ButtonData.OK_DONE) {
				return new Result(uriTextfield.getText(),
